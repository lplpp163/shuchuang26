...
> 這些音檔只提供競賽原型操作示範，不是家人原音。公開使用前，文字會經兩輪 AI 對照檢查中文意思、越南語用字、人物稱呼與情境語氣；家人仍可錄下自己的版本取代示範音。
...
腳本會先核對模型 SHA-256，再由 `bundled_audio_catalog.dart` 讀取 91 條文字，逐檔產生 WAV、用 FFmpeg 轉為 22,050 Hz mono MP3，最後寫出 manifest。產生結果仍須通過資產測試與 AI 內容檢查；hash 一致只證明檔案可以重現，語句自然度仍以對照檢查與家庭修改處理。
...
自動測試、`ffprobe`、瀏覽器媒體事件與 SHA-256 能證明路徑、檔案和播放流程。語句品質另以兩輪 AI 對照檢查，記錄中文意思、越南語用字、人物稱呼、情境語氣與修改前後；需要調整時，直接替換同名 MP3。家庭自己的稱呼與地方說法仍由家人錄製或修改。
